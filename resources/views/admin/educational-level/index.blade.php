        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">All Educational Level</h4>
                    <a class="heading-elements-toggle"><i class="la la-ellipsis-v font-medium-3"></i></a>
                    <div class="heading-elements">
                        <div class="row">
                            <div class="col-lg-12">
                                <a class="btn btn-success" href="{{ route('admin.educational_level.create') }}">Add Educational Level</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-content collapse show">
                    <div class="card-body card-dashboard dataTables_wrapper dt-bootstrap">
                        <table id="department_table" class="table table-striped table-bordered dom-jQuery-events">
                            <thead>
                                <tr>
                                    <th width="10">S. No.</th>
                                    <th>Title</th>
                            ...
                            <tbody>
                                ...
                                <tr data-entry-id="{{ $EducationalLevel->id }}">
                                    <td>{{ $key+1 }}</td>
                                    <td>{{ $EducationalLevel->name ?? '' }}</td>
                                    <td>@if($EducationalLevel->status=='1') Active @else Deactivated @endif</td>
                                    <td>
                                        <a class="btn btn-xs btn-secondary" href="{{ route('admin.educational_level.edit', $EducationalLevel->id) }}"><span class="material-icons">create</span></a>
                                        <form action="{{ route('admin.educational_level.destroy', $EducationalLevel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
